...
function wprig_fonts_url() {
	...
	/**
     *
	 * Translator: If Amatic SC does not support characters in your language, translate this to 'off'.
	 */
	$amatic = esc_html_x( 'on', 'Amatic SC font: on or off', 'wprig' );
	/**
	 * Translator: If PT Mono does not support characters in your language, translate this to 'off'.
	 */
	$pt_mono = esc_html_x( 'on', 'PT Mono font: on or off', 'wprig' );
    /**
	...
	if ( 'off' !== $montserrat ) {
		$font_families[] = 'Montserrat:400,700i';
	}

	if ( in_array( 'on', array( $amatic, $pt_mono, $montserrat ) ) ) {
		$query_args = array(
...
function wprig_styles() {
	...
	wp_enqueue_style( 'wprig-less-main', get_theme_file_uri( '/less/styles.css' ), array(), '20180514' );
...
function wprig_scripts() {
	...
	wp_enqueue_script( 'wprig-script', get_theme_file_uri( '/js/bundle.js' ), array(), '20180514', false);
	wp_script_add_data( 'wprig-script', 'defer', true );
	wp_localize_script( 'wprig-script', 'php_vars', array(
		'home' => get_bloginfo("template_url"),
